  ...
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a href="<?= base_url() ?>account/cashintrnlist" class="btn btn-warning">(x) Clear Filter</a>
      </div>
    </div>
  </div>
</div>
    ...
    <div class="card">
      <div class="card-header flex-wrap border-0 pt-6 pb-0">
        <div class="card-title">
          <h2 class="card-label text-center"><u><b>Treasury Receipt Cash In List </b></u></h2>
        </div>
      </div>
...
<script>
  $(document).ready(function() {
    ...
    function loadAjaxData() {
      $.ajax({
        url: base_url + 'account/get_cashInList',
        type: "POST",
        async: true,
        ...
        success: function(data) {
          ...
          CashInData = data['cash_trn'];
          permission = data['permission'];
          audit_permission = data['audit_permission'];
          swal.close();
          createTables(CashInData, permission, audit_permission);
          return
        },
    ...
    function createTables(CashInData, permission, audit_permission) {
        ...
        columns: [{
          ...
          },
          {
            ...
            render: function(row) {
              ...
              if (permission.view == 1 || permission.edit == 1 || (audit_permission.edit ?? '') == 1) {
                outPut += '<a href="<?php echo base_url() ?>account/editCashinTrn/' + btoa(row.id) + '">' + row.ccode + '</a>'
              } else {
                outPut += row.ccode
              }
              outPut += "</div>"
          ...
          },
          {
            ...
            render: function(row) {
              outPut = "<div>"
              if (permission.view == 1 || permission.edit == 1 || (audit_permission.edit ?? '') == 1) {
                outPut += '<a href="<?php echo base_url() ?>account/editCashinTrn/' + btoa(row.id) + '" class="">' + row.doc_no + '</a>'
              } else {
                outPut += row.doc_no
              }
              outPut += "</div>"
...

          {
            ...
            render: function(data, type, row) {
              ...
              if (permission && permission.edit == '1') {
                action_btn += '<a href="<?php echo base_url() ?>account/editCashinTrn/' + btoa(row.id) + '" class=""><i class="fa fa-pencil"></i> Edit</a>'
              }
              action_btn += '</div>';
              return action_btn
            }
          },
          {
            ...
            render: function(data, type, row) {
              ...
              if (permission && permission.delete == '1') {
                ...
                action_btn += '<a href="<?php echo base_url() ?>account/deleteCashinTrn/' + btoa(row.id) + ' title="delete" class="" onclick="return confirm("' + conf_text + '");"> <i class = "fa fa-times text-danger text"> </i> Delete </a>';
              }
              action_btn += '</div>';
              return action_btn
            }
