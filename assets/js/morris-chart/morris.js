(function() {
  ...
  Morris.Bar = (function(_super) {
    ...
    Bar.prototype.drawXAxis = function() {
      ...
      _results = [];
      for (i = _i = 0, _ref = this.data.length; 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
        row = this.data[this.data.length - 1 - i]; //alert(row.label);
       ...
       // if ((!(prevLabelMargin != null) || (prevLabelMargin >= labelBox.x + labelBox.width) || (prevAngleMargin != null) && prevAngleMargin >= labelBox.x) 
       // && labelBox.x >= 0 && (labelBox.x + labelBox.width) < this.el.width()) {
        if ((!(prevLabelMargin != null) || (prevLabelMargin >= labelBox.x + labelBox.width -50) || (prevAngleMargin != null) && prevAngleMargin >= labelBox.x) 
        && labelBox.x >= 0 && (labelBox.x + labelBox.width) < this.el.width()) {
          ...
          _results.push(label.remove());//heba appear all item text
        }
      }
      return _results;
    };
    ...
    Bar.prototype.drawXAxisLabel = function(xPos, yPos, text) {
      var label;
      //return label = this.raphael.text(xPos, yPos, text).attr('font-size', this.options.gridTextSize).attr('font-family', this.options.gridTextFamily).attr('font-weight', this.options.gridTextWeight).attr('fill', this.options.gridTextColor);//heba
      newText = jQuery(text).text();
      //newText = newText.substr( 0, 10 );
      //newText = newText.split(" ").join("\n");
      return label = this.raphael.text(xPos, yPos, newText).attr('font-size', this.options.gridTextSize).attr('font-family', this.options.gridTextFamily).attr('font-weight', this.options.gridTextWeight).attr('fill', this.options.gridTextColor);
    };
