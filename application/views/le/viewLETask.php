...
<div class="row">
                ...
                <div class="form">
                    ...
                    <table class="table table-striped table-hover table-bordered" id="">
                        <tbody>
                                <tr>
                                    <td>PM Name</td>
                                    <td><?= $this->admin_model->getAdmin($this->db->query("SELECT created_by FROM le_request WHERE id = '$job->request_id'")->row()->created_by);?></td>  
                                    <td>Task Name</td>
                                ...
                                <tr>
                                    <td>Task Code</td>
                                    <td>LE-<?=$job->request_id?>-<?=$job->id?></td>
                                    <td>Assigned LE</td>
                                    <td><?php echo $this->admin_model->getAdmin($job->le) ;?></td>
                                </tr>
                                <tr>
                                    <td>Task Type</td>
                                    <td><?=$this->admin_model->getLETaskType($job->task_type)?></td>
                                     <th>Subject Matter</th>
                                    <td><?=$this->admin_model->getLESubject($job->subject_matter)?></td>
                                </tr>
                                
                                <tr>
                                    <th>Task File</th>
                                    <td><?php if(strlen($job->file) > 1){ ?><a href="<?=base_url()?>assets/uploads/leRequest/<?=$job->file?>" target="_blank">Click Here</a><?php } ?></td>
                                    <td>Product line</td>
                                    <td><?php echo $this->customer_model->getProductLine($priceListData->product_line);?></td>
                                </tr>
                                 <?php if(is_numeric($job->linguist) && is_numeric($job->deliverable)){ ?>
                               <tr>
                                    <td>Linguist Format</td>
                                    <td><?php echo $this->admin_model->getLeFormat($job->linguist);?></td>
                                    <td>Deliverable Format</td>
                                    <td><?php echo $this->admin_model->getLeFormat($job->deliverable);?></td>
                                </tr> 
                            <?php }else{ ?>
                            <tr>
                                <td>Linguist Format</td>
                                <td><?=$job->linguist ?></td>
                                <td>Deliverable Format</td>
                                <td><?=$job->deliverable ?></td>
                            </tr>
                            <?php } ?>  

                                 <tr>
                                    <td>Unit</td>
                                    <td><?php echo $this->admin_model->getUnit($job->unit);?></td>
                                    <td>Volume</td>
                                    <td><?=$job->volume?></td>
                                </tr>
    ...
    <?php if($job->status == 0 && $job->le == $this->user){ ?>
    <div class="row">
    <div class="col-lg-12">
        <section class="panel">
            <header class="panel-heading">
            ...
            <div class="panel-body">
                <div class="form">
                    <form class="cmxform form-horizontal " action="<?php echo base_url()?>le/jobAction" method="post" enctype="multipart/form-data">
    ...
    <div class="col-lg-12">
        <section class="panel">
            ...
            <div class="panel-body">
                <div class="form">
                    <form class="cmxform form-horizontal " action="<?php echo base_url()?>le/reopenJob" method="post" enctype="multipart/form-data">
                        <input name="id" type="hidden" value="<?=base64_encode($job->id)?>" readonly="">
    ...
    <?php if($job->status == 1 && $job->le == $this->user){ ?>
    <div class="row">
    <div class="col-lg-12">
        <section class="panel">
            <header class="panel-heading">
            ...
            <div class="panel-body">
                <div class="form">
                    <form class="cmxform form-horizontal " action="<?php echo base_url()?>le/closeLEJob" method="post" enctype="multipart/form-data">
                        <input name="id" type="hidden" value="<?=base64_encode($job->id)?>" readonly="">
    ...
    <div class="col-lg-12">
        <section class="panel">
            ...
            <?php if($job->status != 2 && $job->status != 4){ ?>
                <div class="panel-body">
                    <div class="form">
                        <form role="form" id="commentForm" action="<?php echo base_url()?>le/jobRespone" method="post" enctype="multipart/form-data">
                          <input name="id" type="hidden" value="<?=base64_encode($job->id)?>" readonly="">
                         <div class="form-group">
                              <label class="col-lg-3 control-label" for="comment">Comment</label>
