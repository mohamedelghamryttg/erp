...
<table id="datatable-fixed-header" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="datatable-fixed-header_info">
    ...
    <tbody>
        <?php 
            foreach ($row as $row) {
            	$closedProjects = $this->db->query(" SELECT j.*,p.customer,p.lead FROM job AS j LEFT OUTER JOIN project AS p ON j.project_id = p.id WHERE j.closed_date BETWEEN '2020-01-01' AND '2021-01-01' AND j.status ='1' AND p.lead = '$row->id' ");
              $totalClosed = 0;
