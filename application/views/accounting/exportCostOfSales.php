    ...
    <tbody>
      <?php if (isset($jobs)) {
        foreach ($jobs->result() as $job) {
          ...
          //Translation
          $translation_tasks = $this->db->select('task_type, unit, created_at, count,tm')->from('translation_request')->where(array('job_id' => $job->id, 'status' => 3))->get()->result();
          $totalRateTrans = 0;
          ...
          <tr>
            <td><?= $this->db->get_where('project', array('id' => $job->project_id))->row()->code; ?> </td>
            <td><?= $job->code ?></td>
            ...
            <?php $extratotal = $this->accounting_model->totalCostByJobCurrency(2, $job->id); ?>
            <td><?= number_format($extratotal, 2) ?></td>

            <td><?= number_format($totalRateTrans, 2); ?></td>
            <td><?= number_format($totalRateLe, 2); ?></td>
            <td><?= number_format($totalRateDtp, 2); ?></td>

            <td><?= number_format(($totalRateDtp + $totalRateLe + $totalRateTrans + $extratotal), 2); ?></td>
