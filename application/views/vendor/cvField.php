	...
	<div class="col-sm-12">
		<section class="panel">
			...
			<div class="panel-body">
			 <form class="cmxform form-horizontal " id="vendorForm" action="<?php echo base_url()?>vendor/cvField" method="post" enctype="multipart/form-data">
				...
				<div class="adv-table editable-table " style="overflow:scroll;">
					<div class="clearfix">
						...
						<?php if($permission->add == 1){ ?>
							<a href="<?=base_url()?>vendor/addCVField?t=<?=base64_encode($idVendorDialect)?>" class="btn btn-primary ">Add New Field</a>
							</br></br></br>
						<?php } ?>
						</div>
					...
					<table class="table table-striped table-hover table-bordered">
						...
						?>
									<tr class="">
                                      <td><?=$row->id?></td>
										<td><a href="<?=base_url()?>vendor/cvFile?t=<?=base64_encode($row->id)?>"><?=$row->field?></a></td>
										<td><?php echo $this->admin_model->getAdmin($row->created_by) ;?></td>
										<td><?php echo $row->created_at ;?></td>
										<td>
											<?php if($permission->edit == 1){ ?>
											<a href="<?php echo base_url()?>vendor/editCVField?t=<?=base64_encode($row->id)?>&x=<?=base64_encode($idVendorDialect)?>" class="">
										...
										<td>
											...
											<a href="<?php echo base_url()?>vendor/deleteCVField?t=<?=base64_encode($row->id)?>&x=<?=base64_encode($idVendorDialect)?>" title="delete" 
