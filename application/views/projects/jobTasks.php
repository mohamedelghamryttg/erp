	...
	<div class="col-sm-12">
		<section class="panel">
			...
			<div class="panel-body">
			  <form class="cmxform form-horizontal " action="<?php echo base_url() ?>projects/closeJob" method="post" enctype="multipart/form-data">
				 <input type="text" name="id" value="<?= base64_encode($job_data->id) ?>" hidden="">
				<div class="form-group">
					<label class="col-lg-2 control-label" for="role name">Close Job</label>

					 <div class="col-lg-3">
						<select name="status" class="form-control m-b" id="status" required="" />
								 <option disabled="disabled" selected="selected" value="">-- Close Job --</option>
				...
				<div class="adv-table editable-table " style="overflow:scroll;">
					...
					<table class="table table-striped table-hover table-bordered" id="">
						...
						<tbody>
							...
							<tr class="">
								<td><a
										href="<?= base_url() ?>projects/projectJobs?t=<?= base64_encode($job_data->project_id) ?>"><?= $job_data->code ?></a>
								</td>
								<td>
								...
								<td>
									<?php echo $job_data->start_date; ?>
								</td>
								<td>
									<?php echo $job_data->delivery_date; ?>
								</td>
								<td>
									<?php echo $this->projects_model->getJobStatus($job_data->status); ?>
								</td>
								<td>
									<?php echo $job_data->closed_date; ?>
								</td>
								<td>
									<?php echo $this->admin_model->getAdmin($job_data->created_by); ?>
								</td>
								<td>
									<?php echo $job_data->created_at; ?>
								</td>
							</tr>
						</tbody>
					</table>
	...
	<div class="row">
		<div class="col-sm-12">
			<section class="panel">
				...
				<header class="panel-heading">
					<button id="button_filter2" onclick="showAndHide('dtpTasks','button_filter2');"
						...
						<div class="clearfix">
							<div class="btn-group">
								<?php if ($permission->add == 1 && $job_data->status == 0) { ?>
									<a href="<?= base_url() ?>projects/dtpRequest?t=<?= base64_encode($job) ?>"
										class="btn btn-primary ">DTP Request</a>
									</br></br></br>
								<?php } ?>
								...
								if (isset($dtp_request)) {
									foreach ($dtp_request->result() as $row) {
										...
										<tr>
											...
											<td>
												<?= $this->admin_model->getDTPTaskType($row->task_type) ?>
											</td>
											<td>
											...
											<td>
												<?= $this->admin_model->getDTPDirection($row->source_direction) ?>
											</td>
											<td>
												<?= $this->admin_model->getDTPDirection($row->target_direction) ?>
											</td>
											<td>
												<?= $this->admin_model->getDTPApplication($row->source_application) ?>
											</td>
											<td>
												<?= $this->admin_model->getDTPApplication($row->target_application) ?>
											</td>
											<td>
												<?= $this->admin_model->getDTPApplication($row->translation_in) ?>
											</td>
											<td>
												<?= $row->rate ?>
											</td>
											<td>
												<?php echo $rateTrnasfared * $row->volume; ?>
											</td>
											<td>
												<?php if (strlen($row->file) > 1) { ?>
													<a href="<?= $this->projects_model->getTaskFileLink("assets/uploads/dtpRequest/", $row->file, $row->start_after_type) ?>"
											...
											<td>
												<?= $this->projects_model->getDTPTaskStatus($row->status) ?>
											</td>
											<td>
											...
											<td>
												<?php if ($permission->edit == 1 && ($row->status == 1 || $row->status == 5)) { ?>
													<a href="<?php echo base_url() ?>projects/cancelDTPRequest?t=<?php echo
													   	base64_encode($row->id); ?>&j=<?= base64_encode($job) ?>" title="Cancel" class=""
											...
											<td>
												<?php if ($permission->delete == 1 && $row->status == 0) { ?>
													<!-- <a href="<?php echo base_url() ?>projects/deleteTask?t=<?php echo
	...
	<div class="row">
		<div class="col-sm-12">
			<section class="panel">
				...
				<header class="panel-heading">
					<button id="button_filter3" onclick="showAndHide('translationTasks','button_filter3');"
						style="padding-top:0pc; float: right;background-color: transparent;font-size: 15px;"
						class="btn btn-sm btn-light text-dark"><i class="fa fa-chevron-down"></i></button>
					Translation Tasks
				</header>
				...
				<div id="translationTasks" class="panel-body">
					<div class="adv-table editable-table " style="overflow-y: scroll;">
						<div class="clearfix">
							<div class="btn-group">
								<?php if ($permission->add == 1 && $job_data->status == 0) { ?>
									<a href="<?= base_url() ?>projects/addTranslationTask?t=<?= base64_encode($job) ?>"
										class="btn btn-primary ">Translation Request</a>
									</br></br></br>
								<?php } ?>
							...
							<tbody>
								...
								if (isset($translation_request)) {
									foreach ($translation_request->result() as $row) {
										...
										$dateArray = explode("-", $row->created_at);
										$year = $dateArray[0];
										$rateProduction = $this->db->get_where('production_team_cost', array('task_type' => $row->task_type, 'unit' => $row->unit, 'year' => $year, 'team' => 1))->row()->rate ?? 0;
										$rateTrnasfared = number_format($this->accounting_model->transfareTotalToCurrencyRate(1, 2, $row->created_at, $rateProduction), 2);
										...
										<tr>
											...
											<td>
												<?php echo $row->count - $row->tm; ?>
											</td>
											<td>
												<?php echo $this->admin_model->getUnit($row->unit); ?>
											</td>
											<td>
												<?php echo number_format($rateTrnasfared * $row->count, 2); ?>
											</td>
											<td>
											...
											<td>
												<?php if (strlen($row->file) > 1) { ?>
													<a href="<?= $this->projects_model->getTaskFileLink("assets/uploads/translationRequest/", $row->file, $row->start_after_type) ?>"
														target="_blank">Click Here</a>
												<?php } else {
													if ($row->start_after_id != null && $row->start_after_type == "Vendor") { ?>
														<?= $this->projects_model->getTaskVendorNotes($row->start_after_id) ?>
											...
											<td>
												<?php if ($permission->edit == 1 && ($row->status == 1 || $row->status == 5)) { ?>
													<a href="<?php echo base_url() ?>projects/editTranslationTask?t=<?php echo
													   	base64_encode($row->id); ?>&j=<?= base64_encode($job) ?>" class="">
														<i class="fa fa-pencil"></i> Edit
													</a>
												<?php } ?>
											...
											<td>
												<?php if ($permission->edit == 1 && ($row->status == 1 || $row->status == 5)) { ?>
													<a href="<?php echo base_url() ?>projects/cancelTranslationRequest?t=<?php echo
													   	base64_encode($row->id); ?>&j=<?= base64_encode($job) ?>" title="Cancel" class=""
														onclick="return confirm('Are you sure you want to cancel this Task ?');">
														<i class="fa fa-times text-danger text"></i> Cancel
													</a>
...
<div class="row">
	<div class="col-sm-12">
		<section class="panel">
			...
			<header class="panel-heading">
				<button id="button_filter1" onclick="showAndHide('vendorTasks','button_filter1');"
					style="padding-top:0pc; float: right;background-color: transparent;font-size: 15px;"
					class="btn btn-sm btn-light text-dark"><i class="fa fa-chevron-down"></i></button>
				Vendor Tasks
			</header>
			<div id="vendorTasks" class="panel-body">
				<div class="adv-table editable-table " style="overflow-y: scroll;">
					<div class="clearfix">
						<div class="btn-group">
							<?php if ($permission->add == 1 && $job_data->status == 0 && $this->brand == 1) { ?>
								<a href="<?= base_url() ?>projects/addTaskVendorModule?t=<?= base64_encode($job) ?>"
									class="btn btn-primary ">Add New Task</a>
								</br></br></br>
							<?php } else { ?>
								<a href="<?= base_url() ?>projects/addTask?t=<?= base64_encode($job) ?>"
									class="btn btn-primary ">Add New Task</a>
								</br></br></br>
							<?php } ?>
					...
					<table class="table table-striped table-hover table-bordered" style="overflow:scroll;">
						...
						<tbody>
							<?php foreach ($task->result() as $row) {
								?>
								<tr>
									<td><a href="<?= base_url() ?>projects/taskPage?t=<?= base64_encode($row->id) ?>"><?= $row->code ?></a>
									</td>
									<td>
										<?php echo $row->subject; ?>
									</td>
									...
									<td>
										<?php if (strlen($row->file) > 1) { ?><a
											href="<?= base_url() ?>assets/uploads/taskFile/<?= $row->file ?>"
											target="_blank">Click Here</a>
									<?php } ?>
									</td>
									<td>
										<?php if (strlen($row->vendor_task_file) > 1) { ?><a
											href="<?= base_url() ?>assets/uploads/vendorTaskFile/<?= $row->vendor_task_file ?>"
											target="_blank">Click Here</a>
									<?php } ?>
									</td>
									<td>
										...
										<?php if ($permission->view == 1 && $row->job_portal == 1 && $row->status == 5) { ?>
											<p class="text-center"> <a href="<?php echo base_url() ?>projects/pmDirectConfirm?task_id=<?php echo
											   	base64_encode($row->id); ?>" class="btn btn-sm btn-default mt-2"
													onclick="return confirm('Are you sure you want to Confirm this Task ?');">
									...
									<td>
										<?php if ($permission->edit == 1 && $row->status == 1 && $row->verified != 1 && $job_data->status == 0 && $this->automation_model->checkIfSoftwareMember($this->emp_id) ) { ?>
											<a href="<?php echo base_url() ?>projects/reopenTask?t=<?php echo
											   	base64_encode($row->id); ?>&p=<?= base64_encode($job_data->status) ?>" class=""
									...
									<td>
										<?php if ($permission->edit == 1 && ($row->status == 0 || $row->status == 4 || $row->status == 3)) { ?>
											<a href="<?php echo base_url() ?>projects/editTask?t=<?php echo
											   	base64_encode($row->id); ?>&j=<?= base64_encode($job) ?>" class="">
												<i class="fa fa-pencil"></i> Edit
											</a>
										<?php } ?>
									...
									<td>
										<?php if ($permission->delete == 1 && $row->status != 1) { ?>
											<a href="<?php echo base_url() ?>projects/deleteTask?t=<?php echo
											   	base64_encode($row->id); ?>&j=<?= base64_encode($job) ?>" title="delete" class=""
									...
									<td>
										<?php if ($permission->edit == 1 && $row->status != 1) { ?>
											<a href="<?php echo base_url() ?>projects/cancelTask?t=<?php echo
											   	base64_encode($row->id); ?>&j=<?= base64_encode($job) ?>" title="Cancel" class=""
												onclick="return confirm('Are you sure you want to Cancel this Task ?');">
												<i class="fa fa-times text-danger text"></i> Cancel
											</a>
...
<div class="row">
	<div class="col-sm-12">
		<section class="panel">
			...
			<header class="panel-heading">
				<button id="button_filter2" onclick="showAndHide('leTasks','button_filter2');"
					style="padding-top:0pc; float: right;background-color: transparent;font-size: 15px;"
					class="btn btn-sm btn-light text-dark"><i class="fa fa-chevron-down"></i></button>
				LE Tasks
			</header>
			...
			<div id="leTasks" class="panel-body">
				<div class="adv-table editable-table " style="overflow-y: scroll;">
					<div class="clearfix">
						<div class="btn-group">
							<?php if ($permission->add == 1 && $job_data->status == 0) { ?>
								<a href="<?= base_url() ?>projects/addLeTask?t=<?= base64_encode($job) ?>"
									class="btn btn-primary ">LE Request</a>
								</br></br></br>
							<?php } ?>
							...
							if (isset($le_request)) {
								foreach ($le_request->result() as $row) {
									...
									<tr>
										...
										<td>
											<?php if (strlen($row->file) > 1) { ?>
												<a href="<?= $this->projects_model->getTaskFileLink("assets/uploads/leRequest/", $row->file, $row->start_after_type) ?>"
													target="_blank">Click Here</a>
											<?php } else {
												if ($row->start_after_id != null && $row->start_after_type == "Vendor") { ?>
													<?= $this->projects_model->getTaskVendorNotes($row->start_after_id) ?>
										...
										<td>
											<?php echo $this->projects_model->getLETaskStatus($row->status); ?>
										</td>
										<td>
											<?php echo $this->admin_model->getAdmin($row->created_by); ?>
										</td>
...
<div class="row">
	<div class="col-sm-12">
		<section class="panel">
			...
			<header class="panel-heading">
				<button id="button_filter2" onclick="showAndHide('Commission','button_filter2');"
					style="padding-top:0pc; float: right;background-color: transparent;font-size: 15px;"
					class="btn btn-sm btn-light text-dark"><i class="fa fa-chevron-down"></i></button>
				Commission
			</header>
			...
			<div id="leTasks" class="panel-body">
				<div class="adv-table editable-table " style="overflow-y: scroll;">
					<div class="clearfix">
						<div class="btn-group">
							<?php if ($permission->add == 1 && $job_data->status == 0) { ?>
								<a href="<?= base_url() ?>projects/addCommission?t=<?= base64_encode($job) ?>"
									class="btn btn-primary ">Add Commission</a>
								</br></br></br>
							<?php } ?>
						...
						<tbody>
							<?php foreach ($project_comission->result() as $row) {
								...
								<tr>
									...
									<td>
										<?php echo $this->projects_model->getCommissionName($row->commission); ?>
									</td>
									<td>
										<?php echo $row->rate; ?>
									</td>
									...
									<td>
										<?php if ($permission->edit == 1) { ?>
											<a href="<?php echo base_url() ?>projects/editCommission?t=<?php echo
											   	base64_encode($row->id); ?>" class="">
												<i class="fa fa-pencil"></i> Edit
											</a>
										<?php } ?>
