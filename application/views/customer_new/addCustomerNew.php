...
<script>
    function AddLead() {
        ...
        leadCount();
        $('html, body').animate({
            scrollTop: $(".card-body .leadCard .multi-collapse.show").offset().top
        });
    }
    ...
    function AddContact(event) {
        ...
        $('html, body').animate({
            scrollTop: $(this).closest(".leadCard").find('.contactCard:last').offset().top
        });
        contactCount();
    }
    ...
    function getCountriesRow(event) {
        ...
        $.post(base_url + "customer/getCountries", {
            region: region
        }, function(data) {
            $('#loading').hide();
