    ...
    <div class="container-fluid">
      ...
      <div class="card card-custom gutter-b example example-compact">
        <div class="card-header">
          <h3 class="card-title">Search Branches</h3>
        </div>
        <?php
        ...
        ?>
        <form class="form" id="branches" action="<?= base_url() ?>customer/customerBranch" method="get"
              ...
              <div class="row">
                ...
                <div class="col-lg-10">
                  <button class="btn btn-success mr-2" name="search"
                    onclick="var e2 = document.getElementById('branches'); e2.action='<?= base_url() ?>customer/customerBranch'; e2.submit();"
                    type="submit">Search</button>
                  <a href="<?= base_url() ?>customer/customerBranch" class="btn btn-warning"><i
                      class="la la-trash"></i>Clear Filter</a>

                  <button class="btn btn-secondary"
                    onclick="var e2 = document.getElementById('branches'); e2.action='<?= base_url() ?>customer/exportbranches'; e2.submit();"
        ...
        <div class="card">
          <div class="card-header flex-wrap border-0 pt-6 pb-0">
            <div class="card-title">
              <h3 class="card-label">Customers Branches</h3>
            </div>
              ...
              <tbody>
                <?php foreach ($branch->result() as $row) {
                  ...
                  <tr class="">
                    ...
                    <td>
                      <a href="<?= base_url() ?>customer/leadContacts?t=<?= base64_encode($row->id) ?>">
                        <?= $this->customer_model->getCustomer($row->customer); ?>
                      </a>
                    </td>
                    <?php if (is_numeric($row->source)) { ?>
                      <td>
                        <?= $this->customer_model->getSource($row->source); ?>
                      </td>
                    <?php } else { ?>
                      <td>
                        <?= $row->source ?>
                    ...
                    <?php } ?>
                    <td>
                      <?= $this->admin_model->getRegion($row->region); ?>
                    </td>
                    <td>
                      <?= $this->admin_model->getCountry($row->country); ?>
                    </td>
                    <td>
                      <?= $this->customer_model->getType($row->type); ?>
                    </td>
                    <td id="SamTable_<?= $row->id ?>">
                      <?php if ($row->approved == 1) { ?>
                        <?php if ($SamCustomer->num_rows() == 0) { ?>
                          <?php if ($permission->follow == 2) { ?>
                            <a href="#myModal<?= $row->id; ?>" data-toggle="modal" class="btn btn-success">Assign SAM</a>
                          <?php } ?>
                        <?php } else { ?>
                            ...
                            foreach ($SamCustomer->result() as $sam) {
                              ...
                              <tr>
                                <td style="border: 1px solid #ddd;">
                                  <?= $this->admin_model->getAdmin($sam->sam); ?>
                                </td>
                                <?php if ($permission->follow == 2) { ?>
                                  <!-- <td style="border: 1px solid #ddd;"><a href="<?= base_url() ?>customer/deleteSamCustomer?t=<?= base64_encode($sam->id); ?>">  -->
                                  <td style="border: 1px solid #ddd;"><a
                                      onclick="deleteSamCustomer(<?= $sam->id ?>,<?= $row->id ?>)">
                                      <i class="fa fa-times text-danger text"></i> </a>
                                  </td>
                                <?php } ?>
                                <?php if ($i < 1) { ?>
                                  <?php if ($permission->follow == 2) { ?>
                                    <td rowspan="<?= $count; ?>" style="border: 1px solid #ddd;">
                                      <a href="#myModal<?= $row->id ?>" data-toggle="modal" class="btn btn-success">Assign SAM</a>
                                    </td>
                                  <?php } ?>
                                <?php } ?>
                      ...
                      <div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1"
                        id="myModal<?= $row->id; ?>" class="modal fade" style="display: none;">
                        <div class="modal-dialog modal-md">
                          <div class=" modal-content">
                            ...
                            <div class="modal-body">
                              ...
                              <input name="lead" id="lead_<?= $row->id ?>" type="hidden" value="<?= $row->id; ?>">
                              <input name="customer" id="customer_<?= $row->id ?>" type="hidden"
                                value="<?= $row->customer; ?>">
                              <div class="form-group">
                                <label for="sam">Select SAM</label>
                                <select name="sam" id="sam_<?= $row->id ?>" class="form-control m-b" required
                                  style="width: 75%;">
                              ...
                              <button class="btn btn-default" type="submit" aria-hidden="true" data-dismiss="modal"
                                class="close" onclick="assignSamCustomer(<?= $row->id ?>)">Submit</button>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <?php } else { ?>
                        <table style="border-collapse:collapse;">
                          ...
                          foreach ($PmCustomer->result() as $pm) {
                            ...
                            <tr>
                              <td style="border: 1px solid #ddd;">
                                <?= $this->admin_model->getAdmin($pm->pm); ?>
                              </td>
                            </tr>
                            <?php
                            $i = $i + 1;
                    ...
                    <td>
                      <?= $this->customer_model->getStatus($row->status); ?>
                    </td>
                    <td>
                      <?php if ($row->approved == 1) {
                        echo "Yes";
                    ...
                    <td>
                      <?php if ($permission->edit == 1) { ?>
                        <a href="<?= base_url() ?>customer/editBranch?t=<?= base64_encode($row->id); ?>" class="">
                    ...
                    <td>
                      <?php if (($permission->delete == 1 && $permission->follow == 2) || ($permission->delete == 1 && $row->created_by == $user)) { ?>
                        <a href="<?= base_url() ?>customer/deleteLead?t=<?= base64_encode($row->id); ?>" title="delete"
